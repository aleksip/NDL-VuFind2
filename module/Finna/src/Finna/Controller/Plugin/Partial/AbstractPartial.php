<?php
/**
 * Abstract base class for partial controller plugins.
 *
 * PHP version 7
 *
 ...
 * @link     https://vufind.org Main Page
 */
class AbstractPartial extends AbstractPlugin implements PartialInterface
{
    protected $template;
    ...
    /**
     * AbstractPartial constructor.
     *
     * @param string $template  Template file.
     * @param string $captureTo The default "capture to" value to set on the child.
     */
    function __construct($template, $captureTo)
    {
    ...
    {
        return $this->getController()->getEvent()->getRouteMatch()->getParam($name);
    }

    /**
     * Returns the all lowercase name of the requested controller.
     *
     * @return string
     */
    public function getControllerName()
    {
        return strtolower($this->getRouteParam('controller'));
    }

    /**
     * Returns the all lowercase name of the requested action.
     *
     * @return string
     */
    public function getActionName()
    {
        return strtolower($this->getRouteParam('action'));
    }
    ...
    {
        if (!$model = $this->getModel($context)) {
            return null;
        }
        $this->getController()->layout()->addChild($model);
